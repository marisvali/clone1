...
type Gui struct {
	...
	viewX          float64
	viewY          float64
	zoomFactor     float64 // num gui pixels necessary to represent a  mat pixel
	lastMouseX     int
	lastMouseY     int
	world          World
	FSys           FS
...
func (g *Gui) Update() error {
	...
	if slices.Contains(keys, ebiten.KeyE) {
	}
	...
	// Translate mouse coords to play region coords.
	...
	if inpututil.IsMouseButtonJustPressed(ebiten.MouseButtonLeft) {
		...
		if g.world.Dragging >= 0 {
			panic("wrong!")
		}

		// Check if there's any brick under the click.
		for i := range g.world.Bricks {
			p := g.world.Bricks[i].PosPixels
			r := image.Rect(p.X, p.Y,
				p.X+g.world.BrickPixelSize,
				p.Y+g.world.BrickPixelSize)

			if Contains(r, image.Point{mX, mY}) {
				g.world.Dragging = i
				g.world.DraggingOrigin = Pt{mX, mY}
				break
			}
		}
	}
	...
	if inpututil.IsMouseButtonJustReleased(ebiten.MouseButtonLeft) {
		if g.world.Dragging >= 0 {
			...
			g.world.Bricks[g.world.Dragging].PosPixels =
				g.world.MatPosToPixelsPos(
					g.world.Bricks[g.world.Dragging].PosMat)
		}
		g.world.Dragging = -1
	}

	if g.world.Dragging >= 0 {
		offsetX := mX - g.world.DraggingOrigin.X
		offsetY := mY - g.world.DraggingOrigin.Y
		g.world.Bricks[g.world.Dragging].PosPixels = g.world.MatPosToPixelsPos(
			g.world.Bricks[g.world.Dragging].PosMat).Plus(Pt{offsetX, offsetY})
	}
...
func Contains(r image.Rectangle, pt image.Point) bool {
